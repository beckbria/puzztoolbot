...
async function registerCommand(
  ...
  await rest
    .put(Routes.applicationGuildCommands(userID, guild.id), {body: []})
    .then(() => console.log('Successfully cleared commands'))
    .catch(console.error)
    .finally(() => {
      rest
        .put(Routes.applicationGuildCommands(userID, guild.id), {
          body: commands,
        })
...
client.once('ready', () => {
  ...
  const delayedRegistration = setInterval(() => {
    ...
    const commands = ALL_COMMANDS.map(cmd => cmd.data.toJSON());
